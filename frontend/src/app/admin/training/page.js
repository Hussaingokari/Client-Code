        ...
        <div style={{ background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border-main)', boxShadow: '0 1px 4px rgba(0,0,0,0.04)', overflow: 'hidden' }}>
          ...
          {loading ? (
            ...
            <div style={{ padding: '60px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '14px', fontWeight: '600', color: 'var(--text-main)', marginBottom: '8px' }}>No trainings yet</div>
              <button onClick={() => setShowForm(true)}
            ...
            trainings.map(t => (
              <div key={t.id} onClick={() => handleSelectTraining(t)}
                ...
                onMouseEnter={e => { if (selected?.id !== t.id) e.currentTarget.style.background = 'var(--bg-app)'; }}
                onMouseLeave={e => { if (selected?.id !== t.id) e.currentTarget.style.background = 'var(--bg-card)'; }}
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                  <div style={{ fontSize: '14px', fontWeight: '700', color: 'var(--text-main)' }}>{t.title}</div>
                  <Badge status={t.status} />
                </div>
                <div style={{ fontSize: '12px', color: 'var(--text-light)', marginBottom: '4px' }}>
                  👨‍🏫 {t.trainer} · <Badge status={t.mode} />
                </div>
                <div style={{ fontSize: '12px', color: 'var(--text-lighter)', marginBottom: '4px' }}>
        ...
        {selected && (
          <div style={{ background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border-main)', boxShadow: '0 1px 4px rgba(0,0,0,0.04)', overflow: 'hidden' }}>
            ...
            {selected.description && (
              <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border-main)', fontSize: '13px', color: 'var(--text-light)', lineHeight: 1.6 }}>
                {selected.description}
              </div>
            )}

            {selected.meetingLink && (
              <div style={{ padding: '10px 20px', borderBottom: '1px solid var(--border-main)', background: '#f0fdf4' }}>
            ...
            <div style={{ padding: '14px 20px' }}>
              ...
              {loadingEnroll ? (
                <div style={{ textAlign: 'center', color: 'var(--text-lighter)', padding: '20px' }}>Loading...</div>
              ) : enrollments.length === 0 ? (
                <div style={{ textAlign: 'center', color: 'var(--text-lighter)', padding: '20px' }}>
                  <div style={{ fontSize: '28px', marginBottom: '8px' }}>📭</div>
                  No enrollments yet
                </div>
              ) : (
                enrollments.map(enr => (
                  <div key={enr.id} style={{ background: 'var(--bg-app)', borderRadius: '10px', padding: '12px', marginBottom: '8px', border: '1px solid var(--border-main)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <div>
                          <div style={{ fontSize: '13px', fontWeight: '600', color: 'var(--text-main)' }}>{enr.employeeName}</div>
                      ...
                      completingId === enr.id ? (
                        <div style={{ marginTop: '10px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                          ...
                          <button onClick={() => setCompletingId(null)}
                            style={{ padding: '7px 12px', background: 'var(--bg-muted)', color: 'var(--text-light)', border: 'none', borderRadius: '7px', fontSize: '12px', cursor: 'pointer' }}>
                            Cancel
                          </button>
                        </div>
                      ) : (
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '20px' }}>
          <div style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '28px', width: '100%', maxWidth: '580px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h2 style={{ fontSize: '18px', fontWeight: '800', color: 'var(--text-main)' }}>Create Training Program</h2>
