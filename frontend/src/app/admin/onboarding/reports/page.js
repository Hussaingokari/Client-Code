...
export default function OnboardingReportsPage() {
    ...
    const statusSegments = [
        { label: 'Not Started', value: data.notStarted, color: 'var(--border-dark)' },
        ...
        <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '20px' }}>
                {STATS.map((s, i) => (
                    <div key={i} style={{ background: 'var(--bg-card)', borderRadius: '12px', padding: '18px', border: '1px solid var(--border-main)', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                        ...
                        <div style={{ fontSize: '28px', fontWeight: '800', color: 'var(--text-main)' }}>{s.value}</div>
                    </div>
                ))}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                ...
                <div style={{ background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border-main)', padding: '20px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                    <div style={{ fontSize: '14px', fontWeight: '700', color: 'var(--text-main)', marginBottom: '18px' }}>
                        Employees by Department
                    </div>
                    <div style={{ display: 'flex', alignItems: 'flex-end', gap: '10px', height: '160px', paddingBottom: '4px' }}>
                        {deptEntries.map(([dept, count], i) => (
                            <div key={dept} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px', height: '100%', justifyContent: 'flex-end' }}>
                                <span style={{ fontSize: '12px', fontWeight: '700', color: 'var(--text-main)' }}>{count}</span>
                                <div style={{
                    ...
                    <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                        {deptEntries.map(([dept]) => (
                            <div key={dept} style={{ flex: 1, fontSize: '11px', color: 'var(--text-light)', textAlign: 'center', wordBreak: 'break-word' }}>{dept}</div>
                        ))}
                    </div>
                </div>
                ...
                <div style={{ background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border-main)', padding: '20px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '18px' }}>
                        <div style={{
                            ...
                            <div style={{ width: '90px', height: '90px', borderRadius: '50%', background: 'var(--bg-card)' }} />
                        </div>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'center', gap: '16px' }}>
                        {statusSegments.map(seg => (
                            <div key={seg.label} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                ...
                                <span style={{ fontSize: '12px', color: 'var(--text-light)' }}>{seg.label}</span>
                            </div>
                        ))}
                    </div>
                </div>
