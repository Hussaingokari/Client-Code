...
function OnboardingBadge({ status }) {
    const map = {
        PENDING: { bg: '#f1f5f9', color: 'var(--text-light)', label: 'Not Started' },
    ...
    return (
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', fontWeight: '600', color: 'var(--text-light)', marginBottom: '8px' }}>
                ...
                <button onClick={openAddForm}
                    style={{ padding: '10px 20px', background: 'var(--bg-sidebar)', color: 'white', border: 'none', borderRadius: '10px', fontSize: '13px', fontWeight: '700', cursor: 'pointer' }}>
                    + Add Employee
                </button>
            </div>
            ...
            <div style={{ display: 'flex', gap: '12px', marginBottom: '20px' }}>
                <div style={{ position: 'relative', flex: 1 }}>
                    ...
                    <input
                        ...
                        style={{ width: '100%', paddingLeft: '38px', paddingRight: '16px', height: '40px', border: '1.5px solid var(--border-main)', borderRadius: '10px', fontSize: '13px', outline: 'none', boxSizing: 'border-box' }}
                    />
                </div>
                <select value={deptFilter} onChange={handleDeptFilterChange}
                    style={{ padding: '0 14px', height: '40px', border: '1.5px solid var(--border-main)', borderRadius: '10px', fontSize: '13px', outline: 'none', background: 'var(--bg-card)' }}>
                    {DEPARTMENTS.map(d => <option key={d} value={d}>{d}</option>)}
                </select>
            </div>

            <div style={{ background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border-main)', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.2fr 1fr 1fr 0.8fr', gap: '16px', padding: '10px 20px', background: 'var(--bg-app)', borderBottom: '1px solid var(--border-main)' }}>
                    {['Employee', 'Department', 'Joining Date', 'Onboarding', 'Actions'].map(h => (
                        <div key={h} style={{ fontSize: '11px', fontWeight: '700', color: 'var(--text-light)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{h}</div>
                    ))}
                </div>
                ...
                {loading ? (
                    ...
                    <div style={{ padding: '60px', textAlign: 'center', color: 'var(--text-lighter)' }}>No employees found</div>
                ) : (
                    <>
                        {displayedEmployees.map((emp) => (
                            <div key={emp.id} style={{ display: 'grid', gridTemplateColumns: '2fr 1.2fr 1fr 1fr 0.8fr', gap: '16px', padding: '13px 20px', borderBottom: '1px solid var(--border-main)', alignItems: 'center' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', minWidth: 0 }}>
                                    ...
                                    <div style={{ minWidth: 0 }}>
                                        <div style={{ fontSize: '13px', fontWeight: '600', color: 'var(--text-main)' }}>{emp.firstName} {emp.lastName}</div>
                                        <div style={{ fontSize: '11px', color: 'var(--text-lighter)' }}>{emp.email}</div>
                                    </div>
                                </div>
                                <div>
                                    <div style={{ fontSize: '13px', color: 'var(--text-main)' }}>{emp.department || '—'}</div>
                                    <div style={{ fontSize: '11px', color: 'var(--text-lighter)' }}>{emp.designation || '—'}</div>
                                </div>
                                <div style={{ fontSize: '13px', color: 'var(--text-light)' }}>{emp.dateOfJoining || '—'}</div>
                                <div><OnboardingBadge status={onboardingByEmpId[emp.id]} /></div>
                                <div style={{ display: 'flex', gap: '10px' }}>
                                    <button onClick={() => openEditForm(emp)} title="Edit"
                                        style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-light)', fontSize: '15px' }}>✏️</button>
                        ...
                        {totalPages > 1 && (
                            <div style={{ padding: '14px 20px', display: 'flex', justifyContent: 'center', gap: '8px', borderTop: '1px solid var(--border-main)' }}>
                                <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0}
                                    style={{ padding: '6px 14px', border: '1px solid var(--border-main)', borderRadius: '6px', fontSize: '12px', fontWeight: '600', color: page === 0 ? 'var(--border-dark)' : '#374151', background: 'var(--bg-card)', cursor: page === 0 ? 'not-allowed' : 'pointer' }}>← Prev</button>
                                <span style={{ padding: '6px 14px', fontSize: '12px', color: 'var(--text-light)' }}>Page {page + 1} of {totalPages}</span>
                                <button onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1}
                                    style={{ padding: '6px 14px', border: '1px solid var(--border-main)', borderRadius: '6px', fontSize: '12px', fontWeight: '600', color: page >= totalPages - 1 ? 'var(--border-dark)' : '#374151', background: 'var(--bg-card)', cursor: page >= totalPages - 1 ? 'not-allowed' : 'pointer' }}>Next →</button>
                            </div>
                        )}
                    </>
                )}
            ...
            {showForm && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '20px' }}>
                    <div style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '28px', width: '100%', maxWidth: '600px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                            <h2 style={{ fontSize: '18px', fontWeight: '800', color: 'var(--text-main)' }}>{editMode ? 'Edit Employee' : 'Add New Employee'}</h2>
                            <button onClick={() => setShowForm(false)} style={{ background: 'none', border: 'none', fontSize: '20px', cursor: 'pointer', color: 'var(--text-lighter)' }}>✕</button>
                        </div>
                        <form onSubmit={handleSubmit}>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '14px' }}>
                                ...
                                <div style={{ position: "relative" }}>
                                    ...
                                    <span onClick={() => setShowPassword(!showPassword)} style={{ position: "absolute", right: "12px", top: "38px", cursor: "pointer", color: "var(--text-light)", fontSize: "16px" }}>
                                ...
                                <div>
                                    ...
                                    <select value={form.department} onChange={e => handleFieldChange('department', e.target.value)} style={{ width: '100%', padding: '9px 12px', border: '1.5px solid var(--border-main)', borderRadius: '8px', fontSize: '13px', outline: 'none', background: 'var(--bg-card)' }}>
                            ...
                            <div style={{ marginBottom: '20px' }}>
                                ...
                                <select value={form.role} onChange={e => setForm({ ...form, role: e.target.value })} style={{ width: '100%', padding: '9px 12px', border: '1.5px solid var(--border-main)', borderRadius: '8px', fontSize: '13px', outline: 'none', background: 'var(--bg-card)' }}>
                            ...
                            <div style={{ display: 'flex', gap: '10px' }}>
                                <button type="button" onClick={() => setShowForm(false)} style={{ flex: 1, padding: '12px', background: 'var(--bg-card)', color: '#374151', border: '1.5px solid var(--border-main)', borderRadius: '10px', fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>Cancel</button>
                                <button type="submit" disabled={submitting} style={{ flex: 1, padding: '12px', background: 'var(--bg-sidebar)', color: 'white', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: '700', cursor: submitting ? 'not-allowed' : 'pointer', opacity: submitting ? 0.7 : 1 }}>
            ...
            {showDeleteConfirm && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '20px' }}>
                    <div style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '28px', width: '100%', maxWidth: '400px', boxShadow: '0 20px 60px rgba(0,0,0,0.2)', textAlign: 'center' }}>
                        <div style={{ fontSize: '48px', marginBottom: '16px' }}>⚠️</div>
                        <h2 style={{ fontSize: '18px', fontWeight: '800', color: 'var(--text-main)', marginBottom: '8px' }}>Delete Employee?</h2>
                        ...
                        <div style={{ display: 'flex', gap: '10px' }}>
                            <button onClick={() => setShowDeleteConfirm(null)} style={{ flex: 1, padding: '12px', background: 'var(--bg-card)', color: '#374151', border: '1.5px solid var(--border-main)', borderRadius: '10px', fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>Cancel</button>
