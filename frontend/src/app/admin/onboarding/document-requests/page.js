...
function RejectModal({ doc, onClose, onConfirm, submitting }) {
    ...
    return (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
            <div style={{ background: 'var(--bg-card)', borderRadius: '12px', padding: '24px', width: '420px', maxWidth: '90%' }}>
                ...
                <div style={{ fontSize: '12px', color: 'var(--text-lighter)', marginBottom: '14px' }}>
                ...
                <textarea
                    ...
                    rows={3}
                    style={{ width: '100%', padding: '10px 12px', border: '1.5px solid var(--border-main)', borderRadius: '10px', fontSize: '13px', outline: 'none', resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit', marginBottom: '16px' }}
                />
                <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                    <button onClick={onClose} disabled={submitting}
                        style={{ padding: '9px 16px', background: 'var(--bg-muted)', color: 'var(--text-light)', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>
                        Cancel
                    </button>
                    <button
...
function DocumentCard({ doc, tab, onApprove, onReject, actingId }) {
    ...
    return (
        <div style={{ background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border-main)', padding: '18px 20px', marginBottom: '12px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
...
export default function DocumentRequestsPage() {
    ...
    return (
        <div>
            ...
            <div style={{ display: 'flex', background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border-main)', padding: '6px', marginBottom: '20px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                {TABS.map(t => (
                    <button
                        key={t.key}
                        ...
                        {t.label}
                        <span style={{
                            background: tab === t.key ? 'rgba(255,255,255,0.25)' : 'var(--border-main)',
                            color: tab === t.key ? 'white' : 'var(--text-light)',
            ...
            {loading ? (
                ...
                <div style={{ background: 'var(--bg-card)', borderRadius: '12px', border: '1px dashed var(--border-main)', padding: '60px', textAlign: 'center' }}>
