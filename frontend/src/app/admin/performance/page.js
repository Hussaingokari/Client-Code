...
function StarRating({ value }) {
  return (
    <div style={{ display: 'flex', gap: '2px' }}>
      {[1,2,3,4,5].map(s => (
        <span key={s} style={{ fontSize: '14px', color: s <= Math.round(value) ? '#f59e0b' : 'var(--border-main)' }}>★</span>
      ))}
      <span style={{ fontSize: '12px', color: 'var(--text-light)', marginLeft: '4px' }}>{value}/5</span>
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          <h1 style={{ fontSize: '22px', fontWeight: '800', color: 'var(--text-main)', marginBottom: '4px' }}>Performance Reviews</h1>
          <p style={{ fontSize: '13px', color: 'var(--text-lighter)' }}>Create and manage employee performance reviews</p>
        </div>
      ...
      {/* Reviews Table */}
      <div className="table-responsive" style={{ background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border-main)', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
        <div className="admin-data-table" style={{ minWidth: '760px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', padding: '10px 20px', background: 'var(--bg-app)', borderBottom: '1px solid var(--border-main)' }}>
        ...
        {loading ? (
          <div style={{ padding: '60px', textAlign: 'center', color: 'var(--text-lighter)' }}>Loading...</div>
        ) : reviews.length === 0 ? (
          <div style={{ padding: '80px', textAlign: 'center' }}>
            <div style={{ fontSize: '48px', marginBottom: '16px' }}>⭐</div>
            <div style={{ fontSize: '16px', fontWeight: '600', color: 'var(--text-main)', marginBottom: '8px' }}>No reviews yet</div>
            <button onClick={() => setShowForm(true)}
          ...
          reviews.map((r, i) => (
            <div key={r.id}
              ...
              <div style={{
                ...
                padding: '14px 20px', borderBottom: '1px solid var(--border-main)', alignItems: 'center',
                background: selected?.id === r.id ? '#eff6ff' : 'white',
              }}
                onMouseEnter={e => { if (selected?.id !== r.id) e.currentTarget.style.background = 'var(--bg-app)'; }}
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <div>
                    <div style={{ fontSize: '13px', fontWeight: '600', color: 'var(--text-main)' }}>{r.employeeName}</div>
                    <div style={{ fontSize: '11px', color: 'var(--text-lighter)' }}>{r.employeeCode}</div>
                  </div>
                </div>
                <div style={{ fontSize: '13px', color: 'var(--text-light)' }}>{r.reviewPeriod}</div>
                <StarRating value={r.overallRating}/>
                <Badge status={r.status}/>
                <div style={{ fontSize: '13px', color: 'var(--text-light)' }}>{r.reviewDate}</div>
              </div>

              {/* Expanded Detail */}
              {selected?.id === r.id && (
                <div style={{ padding: '16px 20px', background: 'var(--bg-app)', borderBottom: '1px solid var(--border-main)' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '12px', marginBottom: '16px' }}>
                    {[
                      ...
                      <div key={s.label} style={{ background: 'var(--bg-card)', borderRadius: '8px', padding: '10px', border: '1px solid var(--border-main)', textAlign: 'center' }}>
                        <div style={{ fontSize: '11px', color: 'var(--text-light)', marginBottom: '4px' }}>{s.label}</div>
                        <div style={{ fontSize: '20px', fontWeight: '800', color: '#1e3a5f' }}>{s.value}</div>
                        <div style={{ fontSize: '10px', color: 'var(--text-lighter)' }}>/ 5</div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                    {[
                      ...
                      <div key={d.label} style={{ background: 'var(--bg-card)', borderRadius: '8px', padding: '12px', border: '1px solid var(--border-main)' }}>
                        <div style={{ fontSize: '11px', fontWeight: '700', color: 'var(--text-light)', marginBottom: '6px' }}>{d.label}</div>
        ...
        {!loading && reviews.length > 0 && (
          <div style={{ padding: '16px 20px', borderTop: '1px solid var(--border-main)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--bg-card)', borderBottomLeftRadius: '12px', borderBottomRightRadius: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button 
                ...
                disabled={page === 0}
                style={{ padding: '6px 12px', border: '1px solid var(--border-main)', borderRadius: '6px', background: page === 0 ? 'var(--bg-app)' : 'white', color: page === 0 ? '#94a3b8' : '#374151', cursor: page === 0 ? 'not-allowed' : 'pointer', fontSize: '13px', fontWeight: '600', transition: 'all 0.2s' }}>
                Previous
              </button>
              <button 
                onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))}
                disabled={page >= totalPages - 1}
                style={{ padding: '6px 12px', border: '1px solid var(--border-main)', borderRadius: '6px', background: page >= totalPages - 1 ? 'var(--bg-app)' : 'white', color: page >= totalPages - 1 ? '#94a3b8' : '#374151', cursor: page >= totalPages - 1 ? 'not-allowed' : 'pointer', fontSize: '13px', fontWeight: '600', transition: 'all 0.2s' }}>
                Next
              </button>
            </div>
          </div>
      ...
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '20px' }}>
          <div style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '28px', width: '100%', maxWidth: '580px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h2 style={{ fontSize: '18px', fontWeight: '800', color: 'var(--text-main)' }}>Create Performance Review</h2>
            ...
            <form onSubmit={handleCreate}>
              ...
              {/* Ratings */}
              <div style={{ background: 'var(--bg-app)', borderRadius: '10px', padding: '16px', marginBottom: '16px' }}>
                <div style={{ fontSize: '13px', fontWeight: '700', color: 'var(--text-main)', marginBottom: '14px' }}>Ratings (1–5)</div>
  ...
  return (
    <div>
      ...
      <input
        ...
        style={{ width: '100%' }}
      />
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', color: 'var(--text-lighter)' }}>
