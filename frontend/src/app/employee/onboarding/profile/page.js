...
function InfoField({ icon, label, value }) {
    return (
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', marginBottom: '20px' }}>
            <span style={{ fontSize: '15px', color: 'var(--text-lighter)', marginTop: '2px' }}>{icon}</span>
            <div>
                <div style={{ fontSize: '12px', color: 'var(--text-lighter)', marginBottom: '2px' }}>{label}</div>
    ...
    if (!onboarding) {
        return (
            <div style={{ background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border-main)', padding: '60px', textAlign: 'center', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                <div style={{ fontSize: '14px', fontWeight: '600', color: 'var(--text-main)' }}>Profile not available yet</div>
            </div>
        );
    }
    ...
    return (
        <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: '20px', alignItems: 'start' }}>
                {/* Left card */}
                <div style={{ background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border-main)', padding: '28px 20px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)', textAlign: 'center' }}>
                    <div style={{
                    ...
                    <div style={{ fontSize: '13px', color: 'var(--text-lighter)', marginBottom: '14px' }}>
                        {onboarding.employeeDesignation || '—'}
                    </div>
                    <div style={{ marginBottom: '10px' }}>
                        <StatusPill status={onboarding.status} />
                    </div>
                    <span style={{ display: 'inline-block', background: 'var(--bg-muted)', color: 'var(--text-light)', padding: '4px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: '600' }}>
                        {onboarding.employeeCode}
                    </span>
                </div>

                {/* Right info panel */}
                <div style={{ background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border-main)', padding: '24px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
