...
function StatCard({ label, value, sub, colorClass, iconClass, icon }) {
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-5 flex flex-col justify-between shadow-sm">
      ...
      <div>
        <div className="text-[28px] font-extrabold text-slate-800 dark:text-white mb-1 leading-none">{value}</div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <StatCard
              ...
              value={unreadCount}
              sub="Unread alerts"
              colorClass="bg-fuchsia-50 dark:bg-fuchsia-500/10 text-fuchsia-500"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
              <h3 className="text-[16px] font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2">
                <Calendar className="w-4 h-4 text-emerald-500" /> Today's Attendance
              </h3>
              
              <div className="flex justify-around mb-6 relative">
                <div className="absolute left-1/2 top-0 bottom-0 w-px bg-slate-100 dark:bg-slate-800 -translate-x-1/2"></div>
                
                <div className="text-center z-10 bg-white dark:bg-slate-900 px-4">
                  <div className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">Check In</div>
                  <div className={`text-[24px] font-black ${todayAtt?.checkIn ? 'text-emerald-500' : 'text-slate-300 dark:text-slate-700'}`}>
                ...
                <div className="text-center z-10 bg-white dark:bg-slate-900 px-4">
                  ...
                  <div className={`text-[24px] font-black ${todayAtt?.checkOut ? 'text-amber-500' : 'text-slate-300 dark:text-slate-700'}`}>
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className={`flex-1 py-3 px-4 rounded-xl text-[13px] font-bold transition-all ${
                    todayAtt?.checkIn 
                      ? 'bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-500 cursor-not-allowed border border-slate-100 dark:border-slate-700/50' 
                      : 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-100 dark:hover:bg-emerald-500/20 border border-emerald-100 dark:border-emerald-500/20 shadow-sm'
                  }`}
                >
                ...
                <button
                  ...
                  className={`flex-1 py-3 px-4 rounded-xl text-[13px] font-bold transition-all ${
                    (todayAtt?.checkOut || !todayAtt?.checkIn)
                      ? 'bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-500 cursor-not-allowed border border-slate-100 dark:border-slate-700/50' 
                      : 'bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-500/20 border border-amber-100 dark:border-amber-500/20 shadow-sm'
                  }`}
                >
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
              <h3 className="text-[16px] font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2">
                <CheckCircle className="w-4 h-4 text-blue-500" /> Leave Balance
              </h3>
                  ...
                  {balance.map((b, i) => (
                    <div key={i}>
                      <div className="flex justify-between items-end mb-2">
                        <span className="text-[13px] font-bold text-slate-700 dark:text-slate-200">{b.leaveType}</span>
                      ...
                      <div className="h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                        <div 
                          className={`h-full rounded-full transition-all duration-500 ${
                            b.leaveType === 'ANNUAL' ? 'bg-blue-500' : 
                            b.leaveType === 'SICK' ? 'bg-emerald-500' : 
                            b.leaveType === 'CASUAL' ? 'bg-amber-500' : 'bg-fuchsia-500'
                          }`}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-[16px] font-bold text-slate-800 dark:text-white flex items-center gap-2">
                  <Clock className="w-4 h-4 text-amber-500" /> Recent Leave Requests
                </h3>
              </div>
                  ...
                  {leaves.slice(0, 4).map((l, i) => (
                    <div key={i} className="flex items-center justify-between py-3 border-b border-slate-50 dark:border-slate-800/50 last:border-0">
                      <div>
                        ...
                        <div className="text-[11px] font-medium text-slate-400 dark:text-slate-500 mt-0.5">{l.startDate} – {l.endDate} &middot; {l.totalDays} day(s)</div>
                      </div>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-[16px] font-bold text-slate-800 dark:text-white flex items-center gap-2">
                  <Bell className="w-4 h-4 text-fuchsia-500" /> Recent Notifications
                </h3>
                {unreadCount > 0 && (
                  <span className="bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 px-3 py-1 rounded-full text-[11px] font-bold tracking-wide">
                    {unreadCount} unread
                  </span>
                )}
              </div>
              ...
              {notifications.length === 0 ? (
                ...
                <div className="space-y-1">
                  {notifications.slice(0, 4).map((n, i) => (
                    <div key={i} className="flex gap-3 items-start py-3 border-b border-slate-50 dark:border-slate-800/50 last:border-0">
                      <div className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${n.isRead ? 'bg-slate-200 dark:bg-slate-700' : 'bg-blue-500'}`} />
                      <div>
