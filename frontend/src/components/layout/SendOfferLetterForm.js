...
export default function SendOfferLetterForm() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="bg-[var(--bg-card)] border border-[var(--border-main)] rounded-lg p-4 sm:p-6">
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-2">
            <Mail size={24} className="text-blue-500" />
            <h2 className="text-lg font-medium text-[var(--text-main)]">Send Offer Letter</h2>
          </div>
          <p className="text-sm text-[var(--text-light)]">Send employment offer letter to selected candidate</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--text-main)] mb-2">Salary</label>
            <input
              type="text"
              name="salary"
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--text-main)] mb-2">Joining Date</label>
            <input
              type="date"
              name="joiningDate"
              min={today}
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--text-main)] mb-2">Reporting To</label>
            <input
              type="text"
              name="reportingTo"
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--text-main)] mb-2">Acceptance Deadline</label>
            <input
              type="date"
          ...
          {/* Attachment Section */}
          <div className="border-t border-[var(--border-main)] pt-4 mt-2">
            <label className="block text-sm font-medium text-[var(--text-main)] mb-3">Attachment</label>

            {/* File Upload Area */}
            <div className="border-2 border-dashed border-[var(--border-main)] rounded-lg p-6 text-center bg-[var(--bg-app)] hover:bg-[var(--bg-muted)] transition cursor-pointer mb-3">
              ...
              <p className="text-sm font-medium text-[var(--text-main)] mb-1">Upload PDF file</p>
              <p className="text-xs text-[var(--text-light)] mb-4">Drag and drop or click to browse</p>
              <input
                type="file"
                accept=".pdf"
            ...
            {fileName && (
              <div className="bg-[var(--bg-app)] border border-[var(--border-main)] rounded-lg p-3 flex items-center justify-between">
                <div className="flex items-center gap-3 flex-1">
                  <svg className="w-5 h-5 text-blue-500" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" clipRule="evenodd" />
                  </svg>
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-[var(--text-main)] truncate">{fileName}</p>
                    <p className="text-xs text-[var(--text-light)]">{fileSize} KB</p>
                  </div>
                </div>
                <button
                  type="button"
          ...
          <div className="flex gap-3 justify-end pt-4 border-t border-[var(--border-main)]">
            <button
              ...
              disabled={loading}
              className="px-4 py-2 border border-[var(--border-main)] rounded-lg text-sm font-medium text-[var(--text-light)] hover:bg-[var(--bg-app)] disabled:opacity-60"
            >
              CANCEL
            </button>
            <button
